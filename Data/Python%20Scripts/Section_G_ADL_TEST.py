...
In=r"file:///Z:/RPA/Python/Input/ADL_I510035_09282022_NP.pdf"
Intermediate=r"Z:\RPA\Python\Intermediate\test.csv"
Out=r"Z:\RPA\Python\output\test.xls"
Inputfile=In
adl_excel_out_name=Out
...
def get_adl_df(adl):
    ...
    try:  
            ...
            adl_df['self']  = adl_df['self'].str.strip() # added these lines for the errors in input pdf data-Bed Mobility [0,  0, ]    [0, 0, 0]      [8,  , ]  [0, 0, 0]   
            adl_df['support']  = adl_df['support'].str.strip()
            adl_df['frequency']  = adl_df['frequency'].str.strip()
...
##ONE3))))))))))                                                                # cond11
    ...
    cond3=all_self_0
    cond4=((sum_freq_0>=3 or sum_freq_1>=3) and (sum_freq_2<3 and sum_freq_3<3 and sum_freq_4<3 ))# and (sum_freq_2+sum_freq_3+sum_freq_4)<3)) 8/16 Heather
    cond5=(all_self_4 or (count_self-count_self_8)==count_self_4)
    #cond6=(sum_freq_3>=3 or(sum_freq_3+sum_freq_4)>=3)
    #cond6=sum_freq_3>=3
    cond6=(sum_freq_3>=3 or sum_freq_4 >=3) # changed on 9/14
    cond7=sum_freq_2>=3
    cond8=sum_freq_1>=3
...
if count==0:
    ...
    df_coma_list=df_coma.index.to_list()
    if len(df_coma) > 0:
            
        for ind in df_coma_list:
                print(ind)
                df.loc[[ind]]=df.loc[[ind]].shift(1, axis=1)
